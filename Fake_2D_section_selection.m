...
if isfield(params, 'skipPlaneCutting') && params.skipPlaneCutting
    fprintf('Skipping Plane Cutting and Alignment as requested.\n');
    % Set defaults for skipped values
    planePosition = [NaN NaN NaN];
    planeNormal = [0 0 0];
    targetAxis = 'Minimal';
    planePosition2 = [NaN NaN NaN];
    dip = 0;
    dipDir = 0;
...
posSliderX = uicontrol('Style', 'slider', 'Position', [980 panel_pos_y_start-255 100 20], ...
                     'Min', min(x), 'Max', max(x), 'Value', planePos(1), ...
                     'SliderStep', [0.001 0.01], 'Callback', @updatePositionFromSliderX);
...
posSliderY = uicontrol('Style', 'slider', 'Position', [980 panel_pos_y_start-285 100 20], ...
                     'Min', min(y), 'Max', max(y), 'Value', planePos(2), ...
                     'SliderStep', [0.001 0.01], 'Callback', @updatePositionFromSliderY);
...
posSliderZ = uicontrol('Style', 'slider', 'Position', [980 panel_pos_y_start-315 100 20], ...
                     'Min', min(z), 'Max', max(z), 'Value', planePos(3), ...
                     'SliderStep', [0.001 0.01], 'Callback', @updatePositionFromSliderZ);
...
% --- Nested Functions ---     
    ...
    function updatePositionFromSliderZ(src, ~)
        val = get(src, 'Value');
        planePos(3) = val;
        updatePlaneVisualization();
    end
    ...
    function updatePlaneVisualization()     
        ...
        if planePos(3) >= get(posSliderZ, 'Min') && planePos(3) <= get(posSliderZ, 'Max')
            set(posSliderZ, 'Value', planePos(3));
        end
...
y_start = y_start - spacing;
% Skip Plane Cutting & Alignment Checkbox
chk_skipCutting = uicontrol('Style', 'checkbox', 'Position', [220 y_start 250 h], ...
    'String', 'Skip Plane Cutting & Alignment', 'Value', false);
